...
try:
    import psycopg2
except ImportError:  # DuckDB-only local dev without RDS deps
...
def get_connection():
    """Return active DB connection (DuckDB staging or PostgreSQL per DATA_BACKEND)."""
    from Executer.data_backend import get_connection as _route_connection

    return _route_connection()


def _postgres_connection():
    if psycopg2 is None:
        raise RuntimeError(
            "psycopg2 is not installed. Use DATA_BACKEND=duckdb or pip install psycopg2-binary"
        )
...
def get_db_schema(conn=None):
    from Executer.data_backend import use_duckdb_staging

    if use_duckdb_staging():
        from Executer.duckdb_store import get_db_schema as _duck_schema

        return _duck_schema(conn)
    return _postgres_get_db_schema(conn)


def _postgres_get_db_schema(conn):
...
def validate_and_normalize_sql(sql_query: str) -> str:
    from Executer.data_backend import sql_dialect

    dialect = sql_dialect()
    logger.debug("Validating SQL (%s):\n%s", dialect, sql_query)
    try:
...
def execute_query(conn, sql_query, max_cost: Optional[float] = None, timeout_ms=60000):
    ...
    if use_duckdb_staging():
        from Executer.duckdb_store import execute_query as _duck_execute

        return _duck_execute(conn, sql_query, timeout_ms=timeout_ms)
    return _postgres_execute_query(conn, sql_query, max_cost, timeout_ms)


def _postgres_execute_query(conn, sql_query, max_cost: Optional[float] = None, timeout_ms=60000):
