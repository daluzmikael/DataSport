...
def _merge_season_folder(
    ...
    for dash_path in dash_files:
        ...
        clutch_path = season_dir / dash_path.name.replace("dash_", "clutch_", 1)
        ...
        hustle_path = season_dir / dash_path.name.replace("dash_", "hustle_", 1)
...
def _stage_shot_zones(root: Path, out_name: str, id_col: str) -> Path:
    ...
    combined = pd.concat(frames, ignore_index=True)
    combined = reorder_slice_context_columns(combined, id_col=id_col)
    STAGING_ROOT.mkdir(parents=True, exist_ok=True)
    combined.to_parquet(out_path, index=False)
    logger.info("wrote %s (%s rows)", out_path, len(combined))
    return out_path
